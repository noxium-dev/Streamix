...
/**
 * Constructs a URL for a Streamix video poster.
 * If the path is relative, it prepends the UPnShare base URL.
 * 
 * @param path - The poster path or URL.
 * @returns A full URL to the image.
 */
export const getStreamixImageUrl = (path?: string): string => {
  if (!path) return "https://dancyflix.com/placeholder.png";
  if (path.startsWith("http")) return path;
  
  // Use the streaming domain for posters
  const base = "https://streamix.upns.pro";
